 ...
 *
 * <p>Uses two flywheel sims for the drive and angle motors, with the absolute position initialized
...
public class ModuleIOSim implements ModuleIO {
  ...
  private DCMotorSim angleSim = new DCMotorSim(DCMotor.getNEO(1), 150.0 / 7.0, 0.004);

  private final Rotation2d angleAbsoluteInitPosition = new Rotation2d(Math.random() * 2.0 * Math.PI);
  ...
  public void updateInputs(ModuleIOInputs inputs) {
    ...
    angleSim.update(LOOP_PERIOD_SECS);
    ...
    inputs.angleAbsolutePosition =
        new Rotation2d(angleSim.getAngularPositionRad()).plus(angleAbsoluteInitPosition);
    inputs.anglePosition = new Rotation2d(angleSim.getAngularPositionRad());
    inputs.angleVelocityRadPerSec = angleSim.getAngularVelocityRadPerSec();
